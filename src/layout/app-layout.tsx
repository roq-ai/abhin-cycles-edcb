...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<ShopInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/shops` : null),
    () => getShops({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Shops',
      path: isTenantUser ? `/shops/view/${data?.[0]?.id}` : '/shops',
      entity: 'shop',
    ...
    { name: 'Cycles', path: '/cycles', entity: 'cycle', service: AccessServiceEnum.PROJECT, icon: FiRotateCcw },
